...
const run = async () => {
  ...
  try {
    ...
    await server.listen(process.env.ADMIN_SERVER_PORT);
    logger.info(`AdminServer is running at http://localhost:${server.get('port')}`);
  } catch (error) {
    logger.error(`AdminServer running error: ${error.message}`);
  }
};
